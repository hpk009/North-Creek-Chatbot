...
const USER_AGENT = "TheBellNorthCreekIndexer/1.0 (+North Creek school information assistant)";
...
async function writeStatus(status: NorthCreekIndexStatus): Promise<void> {
  await ensureDataDir();
  await writeFile(`${STATUS_PATH}.tmp`, JSON.stringify(status, null, 2), "utf8");
...
function extractTitle(html: string, url: string): string {
  ...
  return htmlToText(title || ogTitle || new URL(url).pathname.split("/").filter(Boolean).pop() || "North Creek High School");
}

function extractLastUpdated(html: string): string {
  const pagePublished = html.match(/<meta[^>]+name=["']page-published["'][^>]+content=["']([^"']*)["']/i)?.[1];
  const pageUpdated = html.match(/<meta[^>]+name=["'](?:page-updated|last-modified)["'][^>]+content=["']([^"']*)["']/i)?.[1];
...
function normalizeUrl(rawUrl: string, baseUrl: string): string | null {
  try {
    ...
    if (url.protocol !== "https:" || url.hostname !== "northcreek.nsd.org") return null;
    url.hash = "";
...
function isCrawlablePage(url: string): boolean {
  ...
  return !/\.(?:css|js|json|xml|rss|atom|ics|zip|jpg|jpeg|png|gif|webp|svg|mp4|mov|mp3|woff2?|ttf|pdf)$/i.test(pathname);
}
...
async function fetchRobots(): Promise<RobotsRules> {
  const fallback: RobotsRules = { crawlDelayMs: 5000, allows: [], disallows: [] };
  try {
    ...
    for (const rawLine of lines) {
      ...
      if (key === "crawl-delay" && Number.isFinite(Number(value))) crawlDelayMs = Math.max(1000, Number(value) * 1000);
      if (key === "allow" && value) allows.push(value);
      if (key === "disallow" && value) disallows.push(value);
    }
    return { crawlDelayMs, allows, disallows };
  } catch (error) {
    logger.warn({ error }, "North Creek robots.txt could not be read; using conservative defaults");
    return fallback;
  }
}

async function waitForDelay(lastRequestAt: number, delayMs: number): Promise<void> {
...
function extractFeedCandidates(html: string, pageUrl: string): string[] {
  ...
  for (const match of html.matchAll(/<(?:a|link)\b([^>]+)>/gi)) {
    ...
    if (!href || href === "#" || !/(rss|atom|ical|subscribe|feed)/i.test(marker)) continue;
...
async function confirmCalendarFeed(candidate: string): Promise<boolean> {
  try {
    const response = await fetch(candidate, {
      headers: { accept: "application/rss+xml, application/atom+xml, text/calendar, application/xml", "user-agent": USER_AGENT },
      signal: AbortSignal.timeout(20_000),
    });
    if (!response.ok) return false;
    const contentType = response.headers.get("content-type")?.toLowerCase() || "";
    const sample = (await response.text()).slice(0, 5000).toLowerCase();
    return contentType.includes("rss") || contentType.includes("atom") || contentType.includes("calendar") ||
...
function parseCalendarFeed(text: string, sourceUrl: string): NorthCreekCalendarEvent[] {
  if (/BEGIN:VCALENDAR/i.test(text)) {
    return text.split(/BEGIN:VEVENT/i).slice(1).map((block) => {
      const value = (name: string) => block.match(new RegExp(`^${name}(?:;[^:]*)?:(.*)$`, "mi"))?.[1]?.trim() || "";
      return {
  ...
  return blocks.map((match) => {
    ...
    return {
      ...
      start: xmlValue(block, "start") || xmlValue(block, "dtstart") || xmlValue(block, "pubDate"),
      end: xmlValue(block, "end") || xmlValue(block, "dtend") || xmlValue(block, "pubDate"),
...
function parseCalendarHtml(html: string, sourceUrl: string): NorthCreekCalendarEvent[] {
  const events: NorthCreekCalendarEvent[] = [];
  const titlePattern = /<a\b[^>]*class=["'][^"']*fsCalendarEventTitle[^"']*["'][^>]*>([\s\S]*?)<\/a>/gi;
  for (const match of html.matchAll(titlePattern)) {
    const startIndex = match.index ?? 0;
    const before = html.slice(Math.max(0, startIndex - 4000), startIndex);
    const dateMatches = [...before.matchAll(/class=["']fsCalendarDate["'][^>]*data-day=["'](\d+)["'][^>]*data-year=["'](\d+)["'][^>]*data-month=["'](\d+)["']/gi)];
    const date = dateMatches.at(-1);
    if (!date) continue;
    const after = html.slice(startIndex, startIndex + 2500);
    const nextInfoIndex = after.slice(match[0].length).search(/<div\b[^>]*class=["'][^"']*fsCalendarInfo[^"']*["']/i);
    const eventFragment = nextInfoIndex >= 0
      ? after.slice(0, match[0].length + nextInfoIndex)
      : after;
    const times = [...eventFragment.matchAll(/<time\b[^>]*datetime=["']([^"']+)["']/gi)].map((time) => time[1]);
    const day = `${date[2]}-${String(Number(date[3]) + 1).padStart(2, "0")}-${date[1].padStart(2, "0")}`;
    const location = htmlToText(eventFragment.match(/<div[^>]+class=["'][^"']*fsLocation[^"']*["'][^>]*>([\s\S]*?)<\/div>/i)?.[1] || "");
    events.push({
  ...
  for (const event of events) unique.set(`${event.event_title}|${event.start}`, event);
...
async function performRefresh({ maxPages = DEFAULT_MAX_PAGES }: RefreshOptions): Promise<NorthCreekIndexStatus> {
  ...
  const queue = [NORTH_CREEK_ORIGIN, `${NORTH_CREEK_ORIGIN}/fs/pages/2`, `${NORTH_CREEK_ORIGIN}/our-school/calendar`];
  ...
  try {
    while (queue.length > 0 && pages.size < Math.max(1, Math.floor(maxPages))) {
      ...
      if (new URL(url).pathname === "/our-school/calendar") {
        ...
        for (const candidate of candidates) {
          ...
          if (await confirmCalendarFeed(candidate)) {
            ...
            for (const event of parseCalendarFeed(await feed.text(), candidate)) events.set(`${event.event_title}|${event.start}`, event);
            break;
          }
        }
      }
      ...
      for (const link of extractLinks(fetched.html, url)) {
        if (!visited.has(link) && !queue.includes(link) && isAllowedByRobots(link, robots)) queue.push(link);
      }
    }

    if (!feedConfirmed && calendarHtml) {
      for (const event of parseCalendarHtml(calendarHtml, `${NORTH_CREEK_ORIGIN}/our-school/calendar`)) {
    ...
    await writeStatus(status);
    logger.error({ error, pageCount: pages.size }, "North Creek index refresh failed");
    return status;
  }
}

export function refreshNorthCreekIndex(options: RefreshOptions = {}): Promise<NorthCreekIndexStatus> {
  if (!activeRefresh) {
...
export function startNorthCreekIndexScheduler(): void {
  ...
  void (async () => {
    ...
    const lastCompleted = current.last_completed_at ? Date.parse(current.last_completed_at) : 0;
    if (!lastCompleted || Date.now() - lastCompleted >= REFRESH_INTERVAL_MS) {
      const initialMaxPages = Number(process.env.NORTH_CREEK_INITIAL_MAX_PAGES ?? 30);
...
function isAcademicDishonestyQuestion(question: string): boolean {
  return /\b(cheat|cheating|exam|quiz|test|assignment|homework|essay|paper|worksheet)\b/i.test(question) &&
    /\b(answer|solve|complete|do|finish|give me|help me with|write)\b/i.test(question);
}
...
export async function answerFromNorthCreekIndex(question: string): Promise<{
  ...
  if (isAcademicDishonestyQuestion(question)) {
    return {
      answer: "I can’t complete exams, quizzes, assignments, or other graded work. I can help you find North Creek policies, schedules, or school resources instead.",
      supported: false,
      refusal: "dishonesty",
      sources: [],
    };
  ...
  if (!isNorthCreekScopeQuestion(question)) {
    return {
      answer: "I can only answer questions about North Creek High School using the official North Creek website.",
      supported: false,
      refusal: "scope",
      sources: [],
    };
  ...
  if (!matches.length) {
    return {
      answer: "I couldn’t find supporting information for that question in the North Creek website index.",
      supported: false,
      refusal: "unsupported",
      sources: [],
    };
  ...
  const sources = [...new Set([primary.url, ...matches.map(({ page }) => page.url)])].slice(0, 3);
  return {
...
export async function getNorthCreekIndexSummary(): Promise<{
  ...
  const [status, pages, events] = await Promise.all([getNorthCreekIndexStatus(), readPages(), readEvents()]);
  return {
    status,
    samplePages: pages.slice(0, 10).map(({ url, title, section }) => ({ url, title, section })),
